                ...
                @foreach ($events as $event)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-4 text-sm flex gap-2 justify-end items-center">
                            <form id="accept-form-{{ $event->id }}"
                                action="{{ route('events.accept', $event->id) }}" method="POST" class="mt-3">
                                @csrf
                                <button type="submit"
                                    class="accept-button px-4 py-2 bg-gradient-to-r from-pink-400 rounded text-white text-sm tracking-wider font-medium border-none outline-none bg-pink-600 active:from-pink-500">
                                    Accept
                                </button>
                            </form>
                            <form id="refuse-form-{{ $event->id }}"
                                action="{{ route('events.refuse', $event->id) }}" method="POST" class="mt-3">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2  bg-gradient-to-r from-purple-400 rounded text-white text-sm tracking-wider font-medium border-none outline-none bg-purple-600 active:from-purple-500">
                                    Refuse
                                </button>
                            </form>
                        </td>
